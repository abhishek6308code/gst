// /**
//  * server.js
//  * Node + Express + MongoDB (native driver)
//  *
//  * Endpoints:
//  *  - GET  /api/health
//  *  - POST /api/enquiries
...
//  *  - Creates unique sparse index on phone to prevent duplicates
...
// if (!MONGODB_URI) {
//   console.error('MONGODB_URI is not set. Copy .env.example -> .env and set it.');
//   process.exit(1);
// }
...
//   // keep leading +, remove other non-digit characters
//   return v.replace(/[^\d+]/g, '');
// }
// function isPhoneValid(v) {
//   const p = normalizePhone(v);
//   const digits = p.replace(/\D/g, '');
//   // require between 7 and 15 digits (international friendly)
...
//     db = client.db(); // DB name comes from URI (e.g. gstdb)
//     console.log('Connected to MongoDB');

//     // Ensure unique index on phone for both collections.
//     // sparse: true allows documents without phone field (optional). Remove sparse if phone must always be present.
...
//   console.log('SIGINT: closing MongoDB client');
//   try { await client.close(); } catch (ignored) {}
//   process.exit(0);
// });
// process.on('SIGTERM', async () => {
//   console.log('SIGTERM: closing MongoDB client');
//   try { await client.close(); } catch (ignored) {}
//   process.exit(0);
// });
/**
 * server.js
 * Node + Express + MongoDB (native driver) with admin auth (JWT)
 */
